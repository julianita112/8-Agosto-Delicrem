...
export function ProductoTerminado() {
  ...
  const handlePedidoChange = async (numero_pedido) => {
    ...
    if (pedido) {
      setProductionDetails(pedido.detallesPedido.map(detalle => ({
                        ...
                        <label className="inline-flex relative items-center cursor-pointer">
                          ...
                          <div
                            className={`w-11 h-6 bg-gray-200 rounded-full peer peer-focus:ring-4 peer-focus:ring-blue-300 dark:peer-focus:ring-blue-800 ${
                              producto.activo ? 'peer-checked:bg-green-600' : 'peer-checked:bg-red-600'
                            }`}
                          ></div>
                          <span
                            className={`absolute left-1 top-1 bg-white border border-gray-300 rounded-full h-4 w-4 transition-transform ${
                              producto.activo ? 'translate-x-full border-green-600' : 'translate-x-0 border-red-600'
                            }`}
                          ></span>
                        </label>
                      </td>
            ...
            {productionDetails.map((detalle, index) => (
              <div key={index} className="mb-4 flex items-center">
                ...
                <div className="flex items-center ml-4">
                  <IconButton className="btncancelarm" size="sm" onClick={() => handleRemoveProductionDetalle(index)}>
